  ...
  return (
    <SafeAreaView className="flex-1 bg-background">
      <View className="px-5 pt-4 pb-6">
        <Text className="text-white/40 text-xs font-bold uppercase tracking-[0.3em] mb-4">
          Financial Ledger
        </Text>
        <Text className="text-4xl font-black text-white tracking-tighter mb-6">Activity</Text>
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} className="flex-row">
          {filters.map((f) => (
            ...
            >
              <Text className={`font-bold text-sm ${filter === f.id ? 'text-white' : 'text-white/40'}`}>
                {f.label}
              </Text>
            </TouchableOpacity>
          ))}
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <View className="py-20 items-center">
            <MaterialCommunityIcons name="clipboard-text-outline" size={64} color="rgba(255,255,255,0.05)" />
            <Text className="text-white/20 mt-4 font-bold text-lg">No transactions found</Text>
          </View>
        }
        renderItem={({ item: tx }) => (
          <TouchableOpacity 
          ...
          >
            <View className="flex-row items-center flex-1">
              <View className="w-14 h-14 bg-white/5 rounded-2xl items-center justify-center mr-4">
              ...
              <View className="flex-1">
                <Text className="text-white font-bold text-lg" numberOfLines={1}>{tx.description}</Text>
                <Text className="text-white/30 text-xs font-medium uppercase tracking-widest mt-1">
            ...
            <View className="items-end ml-4">
              ...
              <View className="bg-white/5 px-2 py-0.5 rounded-md mt-1">
                <Text className="text-white/20 text-[9px] font-black uppercase tracking-widest">Completed</Text>
              </View>
            </View>
          </TouchableOpacity>
        )}
