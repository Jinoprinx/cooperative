  ...
  return (
    <div className="space-y-10 pb-20 text-primary-text">
      ...
      <div className="card-premium relative overflow-hidden group">
        ...
        <h2 className="text-xl font-black tracking-tighter mb-8 text-primary-text">Generate Protocol</h2>
        
        <div className="grid gap-6 sm:grid-cols-3 items-end">
          <div className="space-y-2">
            <label className="text-[10px] font-black text-tertiary-text uppercase tracking-widest ml-4">Report Classification</label>
            <select
              value={reportType}
            ...
            >
              <option value="monthly" className="bg-background">Monthly Snapshot</option>
              <option value="yearly" className="bg-background">Annual Audit</option>
            </select>
          </div>

          {reportType === 'monthly' && (
            <div className="space-y-2">
              <label className="text-[10px] font-black text-tertiary-text uppercase tracking-widest ml-4">Select Cycle</label>
              <input
                type="month"
                value={selectedMonth}
                onChange={(e) => setSelectedMonth(e.target.value)}
                className="w-full bg-surface border border-border rounded-2xl p-4 text-primary-text text-sm outline-none focus:border-primary transition-all font-black uppercase tracking-widest"
              />
            </div>
          )}
          ...
          {reportType === 'yearly' && (
            <div className="space-y-2">
              <label className="text-[10px] font-black text-tertiary-text uppercase tracking-widest ml-4">Select Period</label>
              <input
                type="number"
                min="2000"
                max="2100"
          ...
          <button 
            ...
            className="btn-primary py-4 rounded-2xl text-[10px] font-black uppercase tracking-[0.4em] shadow-[0_0_30px_rgba(59,130,246,0.1)] hover:tracking-[0.6em] transition-all duration-500 border-none"
          >
            Synthesize Report
          </button>
        </div>
      ...
      {reportData && (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          <div className="card-premium h-full flex flex-col justify-between border-emerald-500/10">
            ...
            <p className="text-3xl font-black text-emerald-500 tracking-tighter shadow-glow-sm">{formatCurrency(reportData.totalDeposits)}</p>
          </div>
          ...
          <div className="card-premium h-full flex flex-col justify-between">
            ...
            <p className="text-3xl font-black text-primary-text tracking-tighter shadow-glow-sm">{formatCurrency(reportData.totalLoanDisbursements)}</p>
          </div>
          ...
          <div className="card-premium h-full flex flex-col justify-between bg-emerald-500/5 border-emerald-500/20">
            ...
            <p className="text-3xl font-black text-emerald-500 tracking-tighter shadow-glow-lg">{formatCurrency(reportData.netCashFlow)}</p>
          </div>
          
          <div className="card-premium h-full flex flex-col justify-between border-primary/10">
            <h3 className="text-[10px] font-black text-tertiary-text uppercase tracking-[0.4em] mb-4">Total Membership</h3>
            <p className="text-3xl font-black text-primary-text tracking-tighter">{reportData.totalMembers}</p>
          </div>
