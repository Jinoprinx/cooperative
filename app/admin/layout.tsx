  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white selection:bg-primary selection:text-white">
      ...
      >
        ...
        <div className="fixed inset-y-0 left-0 flex w-full max-w-xs flex-col glass-card border-r border-white/5 pt-5 pb-4">
          <div className="px-6 mb-8 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="text-xl font-black tracking-tighter text-white truncate max-w-[150px]">{tenant?.name || 'Admin'}</div>
            </div>
            <button
              type="button"
              className="flex h-10 w-10 items-center justify-center rounded-2xl bg-white/5 border border-white/10 text-white"
            ...
            <nav className="space-y-2">
              {navigation.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon className={`mr-3 h-5 w-5 transition-colors ${isActive ? 'text-white' : 'text-white/40 group-hover:text-white'}`} aria-hidden="true" />
                    {item.name}
                  </Link>
                );
              })}
              ...
              <div className="pt-4 mt-4 border-t border-white/5">
                <button
                  ...
                  className="w-full group flex items-center rounded-2xl px-4 py-3.5 text-sm font-bold text-red-400 hover:bg-red-500/10 transition-all duration-300"
                >
      ...
      <div className="hidden lg:fixed lg:inset-y-0 lg:flex lg:w-72 lg:flex-col z-20">
        <div className="flex min-h-0 flex-1 flex-col glass-card border-r border-white/5 m-4 rounded-[2.5rem]">
          <div className="flex flex-1 flex-col overflow-y-auto pt-8 pb-4 px-6 custom-scrollbar">
            <div className="flex flex-col items-center mb-10">
              ...
              <div className="text-2xl font-black text-white text-center tracking-tighter line-clamp-2 px-2 uppercase leading-none">
                {tenant?.name || 'Admin'}
              </div>
            ...
            <nav className="flex-1 space-y-1.5 pt-4">
              {navigation.map((item) => {
                ...
                return (
                  ...
                  >
                    <item.icon className={`mr-4 h-5 w-5 transition-all duration-300 ${isActive ? 'text-white' : 'text-white/30 group-hover:text-white group-hover:scale-110'}`} aria-hidden="true" />
                    {item.name}
                  </Link>
                );
              })}
            ...
            <div className="mt-auto pt-6 border-t border-white/5">
              <button
                onClick={handleSignOut}
                className="w-full group flex items-center rounded-2xl px-5 py-4 text-sm font-bold text-red-500/80 hover:text-red-400 hover:bg-red-500/5 transition-all duration-300"
              >
      ...
      <div className="flex flex-1 flex-col lg:pl-80 transition-all duration-300">
        <div className="sticky top-0 z-10 flex h-20 flex-shrink-0 backdrop-blur-xl border-b border-white/5 bg-black/20 px-4 sm:px-8">
          <button
            type="button"
            className="px-4 text-white/50 hover:text-white lg:hidden transition-colors"
            onClick={toggleSidebar}
          >
          ...
          <div className="flex flex-1 justify-between items-center h-full">
            ...
            <div className="flex items-center gap-6">
              <div className="hidden sm:flex flex-col items-end">
                <span className="text-white/40 text-[10px] font-black uppercase tracking-widest leading-none mb-1">Welcome back</span>
                <span className="text-sm font-bold text-white leading-none capitalize">{user?.firstName} {user?.lastName}</span>
              </div>
